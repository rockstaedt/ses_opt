...
def objective(u, p1, pg, p2, net_i):
    """
    This function calculates the objective value for all hours in hours. This is
    also the upper bound of the decomposition.
    """
    return sum(
        c1*u[h] + l1*p1[h] + c2*pg[h] + l2*p2[h] + stor_p*net_i[h] for h in HOURS
    )

def master_prob(u, p1, net_i, alpha):
    """
    This function calculates the lower bound of the decomposition.
    """
    return sum(c1*u[h] + l1*p1[h] + stor_p*net_i[h] + alpha[h] for h in HOURS)
...
for l2 in l2s:
    ...
    # net injection from storage with bounds of maximum charge and discharge
    ...
    def con_load(sub, H):
        return sub.pg[H] + sub.p1[H] + sub.p2[H] + sub.stor_net_i[H] >= LOADS[H]
    sub.con_load = pyo.Constraint(sub.H, rule=con_load)

    # initialization for forward contract
    def p2_zero(master):
        return sub.p2[0] == 0
    sub.p2_zero = pyo.Constraint(rule=p2_zero)
    ...
    def con_ramping(sub, H):
        if H != 0:
            ...
            # because of constraint con_max we dont need this
            return sub.pg[H] == 0
    ...
    for key, value in results_sub[0].items():
        print(key, value)
    ...
    while not converged:
        ...
        def cut(master, H):
            return (
                ...
                + stor_p*results_sub[0]['stor_net_i'][H]
